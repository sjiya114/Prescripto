...
import { asyncAdminDashBoard, asyncAuthAdmin } from '../../redux/action/adminAction';
...
function Dashboard() {
 ...
 const {dashdata,docId}=useSelector((state)=>state.adminAuth);
    useEffect(()=>
         {
          ...
          useEffect(()=>
               {
               if(Object.keys(dashdata).length === 0){
               dispatch(asyncAdminDashBoard());
               }
               },[dispatch,dashdata])
  ...
  return (
   ...
   {dashdata && <> <div className='mt-30 mx-10  flex flex-row max-md:flex-col max-md:space-y-4  w-full justify-between max-md:justify-start'>
      <button className='px-10 py-6 space-x-6 rounded-md bg-white flex flex-row'>
        ...
        <h1>{dashdata.doctors} Doctors</h1>
      </button>
      <button className='px-10 py-6 space-x-6 rounded-md bg-white flex flex-row'>
    ...
    <br />
      <div className='flex bg-white flex-col border-gray-500 border-2 rounded-lg space-y-4 ml-10'>
        ...
        <div className='flex flex-col'>
          ...
          {Array.isArray(dashdata.latestAppointments) && dashdata.latestAppointments.map((data,index)=>(<div key={index+'x'} className='flex flex-row px-4 py-2 w-full justify-between'>
